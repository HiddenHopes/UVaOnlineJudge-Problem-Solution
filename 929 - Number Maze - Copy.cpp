...
void dijkstra(iii s, vector<vi> *dist, vector<vi> *graph){     // * used to point dist and graph
...
int main(){
    ...
    while(n--){
        ...
        vi rowIni(M,INF);                   // one row, and M number of columns each assigned with 1000000000
        vector<vi> dist (N,rowIni);         // initialize : N rows, each assigned with a single row rowIni

        dist[0][0] = graph[0][0];
        dijkstra(iii(graph[0][0],ii(0,0)),&dist,&graph);    //use of iii to parse a int and a pair of int
                                                            //& is used to parse dist and graph of vector< vector <int> >
        printf("%d\n",dist[N-1][M-1]);
